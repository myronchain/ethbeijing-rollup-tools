...
export function formatError(error: any, network?: Network) {
  ...
  if (
    ...
    errMsg = `${errMsg} Please check your wallet network settings are correct and try again.`
  } else if (
    errMsg.includes('[ethjs-query]') ||
    errMsg.includes('while formatting outputs from RPC')
  ) {
    errMsg = `An RPC error occured. Please check your wallet network settings are correct and refresh page to try again. Error: ${errMsg}`
  } else if (
    ...
    errMsg = `There was a network error. Please disable any ad blockers and check your wallet network settings are correct and refresh page to try again. Error: ${errMsg}`
  } else if (errMsg.includes('Internal JSON-RPC error') || errMsg.includes('Internal error')) {
    const feeToken = network?.nativeTokenSymbol || 'funds'
    errMsg = `An RPC error occured. Please check you have enough ${feeToken} to pay for fees and check your wallet network settings are correct. Refresh to try again. Error: ${errMsg}`
...
export function fixedDecimals(amount: string, decimals = 18) {
  if (amount === '') {
    return amount
  }
...
export function amountToBN(amount: string | number, decimals = 18) {
